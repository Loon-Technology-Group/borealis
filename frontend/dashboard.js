(function () {
  ...
  function fetchLatest() { return fetchJSON("/api/latest"); }
  function fetchKp(w) { return fetchJSON("/api/kp?window=" + w); }
  function fetchBz(w) { return fetchJSON("/api/bz?window=" + w); }
  function fetchWind(w) { return fetchJSON("/api/wind?window=" + w); }

  function fetchAll(w) {
    return Promise.all([fetchLatest(), fetchKp(w), fetchBz(w), fetchWind(w)])
      .then(function (results) {
        return { latest: results[0], kp: results[1], bz: results[2], wind: results[3] };
      });
  }
  ...
  function renderStatus(latest) {
    ...
    if (latest.bz) {
      ...
      var arrow = bzVal >= 0 ? "\u2191" : "\u2193";
      var bzColor = bzVal >= 0 ? getCssVar("--aurora-teal") : getCssVar("--aurora-purple");
      bzBlock.querySelector(".stat-value").textContent = bzVal.toFixed(1) + " nT " + arrow;
    ...
    if (latest.wind) {
      ...
      windBlock.querySelector(".stat-value").textContent =
        latest.wind.speed.toFixed(0) + " km/s  " + latest.wind.density.toFixed(1) + " p/cm\u00B3";
  ...
  function toUplotData(rows, fields) {
    ...
    var series = fields.map(function () { return new Array(rows.length); });
    ...
    var opts = {
      ...
      hooks: {
        ...
        drawSeries: [function (u, si) {
          ...
          ctx.save();
          var barWid = Math.max(1, Math.round((u.bbox.width / data0.length) * 0.7));
  ...
  function buildAllCharts() {
    ...
    if (state.kpData && state.kpData.length > 0) {
      state.charts.kp = buildKpChart("chart-kp-container", state.kpData, sizes.kp);
    } else {
      showChartError("chart-kp-container", state.kpData ? "No data for selected window" : "Unable to load data");
    }
    if (state.bzData && state.bzData.length > 0) {
      state.charts.bz = buildBzChart("chart-bz-container", state.bzData, sizes.bz);
    } else {
      showChartError("chart-bz-container", state.bzData ? "No data for selected window" : "Unable to load data");
    }
    if (state.windData && state.windData.length > 0) {
      state.charts.wind = buildWindChart("chart-wind-container", state.windData, sizes.wind);
    } else {
      showChartError("chart-wind-container", state.windData ? "No data for selected window" : "Unable to load data");
    }
  }
  ...
  function updateChartData() {
    ...
    if (state.charts.wind && state.windData && state.windData.length > 0) {
      state.charts.wind.setData(toUplotData(state.windData, ["speed", "density"]));
    }
  }
  ...
  function setWindow(w) {
    state.window = w;
    document.querySelectorAll(".window-selector button").forEach(function (btn) {
